...
(function () {
    ...
    timezoneJS = "undefined" != typeof exports ? exports : root.timezoneJS = {}, timezoneJS.VERSION = "0.4.4";
    for (var $ = root.$ || root.jQuery || root.Zepto, fleegix = root.fleegix, _arrIndexOf, DAYS = timezoneJS.Days = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"], MONTHS = timezoneJS.Months = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"], SHORT_MONTHS = {}, SHORT_DAYS = {}, EXACT_DATE_TIME = {}, TZ_REGEXP = new RegExp("^[a-zA-Z]+/"), i = 0; i < MONTHS.length; i++)SHORT_MONTHS[MONTHS[i].substr(0, 3)] = i;
    for (i = 0; i < DAYS.length; i++)SHORT_DAYS[DAYS[i].substr(0, 3)] = i;
    _arrIndexOf = Array.prototype.indexOf || function (a) {
    ...
    var _fixWidth = function (a, b) {
        if ("number" != typeof a)throw"not a number: " + a;
        var c = a.toString();
        if (a.length > b)return a.substr(a.length - b, a.length);
        for (; c.length < b;)c = "0" + c;
        return c
    }, _transport = function (a) {
        if (!(fleegix && "undefined" != typeof fleegix.xhr || $ && "undefined" != typeof $.ajax))throw new Error("Please use the Fleegix.js XHR module, jQuery ajax, Zepto ajax, or define your own transport mechanism for downloading zone files.");
        if (a) {
            if (!a.url)throw new Error("URL must be specified");
            return"async"in a || (a.async = !0), a.async ? fleegix && fleegix.xhr ? fleegix.xhr.send({url: a.url, method: "get", handleSuccess: a.success, handleErr: a.error}) : $.ajax({url: a.url, dataType: "text", method: "GET", error: a.error, success: a.success}) : fleegix && fleegix.xhr ? fleegix.xhr.doReq({url: a.url, async: !1}) : $.ajax({url: a.url, async: !1}).responseText
        }
    };
    timezoneJS.Date = function () {
        var a = Array.prototype.slice.apply(arguments), b = null, c = null, d = [];
        switch ("[object Array]" === Object.prototype.toString.call(a[0]) && (a = a[0]), "string" == typeof a[a.length - 1] && TZ_REGEXP.test(a[a.length - 1]) && (c = a.pop()), a.length) {
            case 0:
                b = new Date;
                break;
            case 1:
                ...
                break;
            default:
                for (var e = 0; 7 > e; e++)d[e] = a[e] || 0;
                b = new Date(d[0], d[1], d[2], d[3], d[4], d[5], d[6])
        }
        this._useCache = !1, this._tzInfo = {}, this._day = 0, this.year = 0, this.month = 0, this.date = 0, this.hours = 0, this.minutes = 0, this.seconds = 0, this.milliseconds = 0, this.timezone = c || null, d.length ? this.setFromDateObjProxy(b) : this.setFromTimeProxy(b.getTime(), c)
    }, timezoneJS.Date.prototype = {getDate: function () {
        return this.date
    }, getDay: function () {
        return this._day
        ...
        return this._timeProxy + 1e3 * 60 * this.getTimezoneOffset()
        ...
        var a;
        return a = this.timezone ? "Etc/UTC" === this.timezone || "Etc/GMT" === this.timezone ? {tzOffset: 0, tzAbbr: "UTC"} : timezoneJS.timezone.getTzInfo(this._timeProxy, this.timezone) : {tzOffset: this.getLocalOffset(), tzAbbr: null}, this._tzInfo = a, this._useCache = !0, a
        ...
        return a = Number(a), a >= 0 && 99 >= a && (a += 1900), this.setUTCAttribute("year", a), this.getTime()
        ...
        var c, d = new Date(a);
        c = b ? timezoneJS.timezone.getTzInfo(d, b).tzOffset : d.getTimezoneOffset(), d.setTime(a + 6e4 * (d.getTimezoneOffset() - c)), this.setFromDateObjProxy(d)
        ...
        var d;
        b++, b > 12 && (d = parseInt(b / 12, 10), b %= 12, a += d), 2 >= b && (a -= 1, b += 12), d = Math.floor(a / 100);
        ...
        function builtInLoadZoneFile(a, b) {
            ...
            return b && b.async ? _this.transport({async: !0, url: c, success: function (a) {
                return _this.parseZones(a) && "function" == typeof b.callback && b.callback(), !0
            }, error: function () {
        ...
        function getRegionForTimezone(a) {
            ...
            var e = _this.zones[a];
            return"string" == typeof e ? getRegionForTimezone(e) : _this.loadedZones.backward ? (invalidTZError(a), void 0) : (_this.loadZoneFile("backward"), getRegionForTimezone(a))
        }

        function parseTimeString(a) {
            var b = /(\d+)(?::0*(\d*))?(?::0*(\d*))?([wsugz])?$/, c = a.match(b);
            return c[1] = parseInt(c[1], 10), c[2] = c[2] ? parseInt(c[2], 10) : 0, c[3] = c[3] ? parseInt(c[3], 10) : 0, c
        }
        ...
        function processZone(a) {
            if (a[3]) {
                ...
                var e = a[6] ? a[6] : "00:00:00", f = parseTimeString(e);
                return[b, c, d, f[1], f[2], f[3]]
            }
        }
        ...
        function getBasicOffset(a) {
            ...
            return b = c * 1e3 * (60 * (60 * b[1] + b[2]) + b[3]), b / 60 / 1e3
        }

        function getRule(a, b, c) {
            var d = "number" == typeof a ? new Date(a) : a, e = b[1], f = b[0], g = e.match(/^([0-9]):([0-9][0-9])$/);
            if (g)return[-1e6, "max", "-", "Jan", 1, parseTimeString("0:00"), 60 * parseInt(g[1]) + parseInt(g[2]), "-"];
            var h, i = function (a, b, c) {
                var d = 0;
                if ("u" === b || "g" === b || "z" === b)d = 0; else if ("s" === b)d = f; else {
                    if ("w" !== b && b)throw"unknown type " + b;
                    d = getAdjustedOffset(f, c)
                }
                ...
                if (EXACT_DATE_TIME[d] || (EXACT_DATE_TIME[d] = {}), EXACT_DATE_TIME[d][e])c = EXACT_DATE_TIME[d][e]; else {
                    if (isNaN(e[4])) {
                        var g, h;
                        "last" === e[4].substr(0, 4) ? (c = new Date(Date.UTC(d, SHORT_MONTHS[e[3]] + 1, 1, f[1] - 24, f[2], f[3], 0)), g = SHORT_DAYS[e[4].substr(4, 3)], h = "<=") : (c = new Date(Date.UTC(d, SHORT_MONTHS[e[3]], e[4].substr(5), f[1], f[2], f[3], 0)), g = SHORT_DAYS[e[4].substr(0, 3)], h = e[4].substr(3, 2));
                        var j = c.getUTCDay();
                        ">=" === h ? c.setUTCDate(c.getUTCDate() + (g - j + (j > g ? 7 : 0))) : c.setUTCDate(c.getUTCDate() + (g - j - (g > j ? 7 : 0)))
                    } else c = new Date(Date.UTC(d, SHORT_MONTHS[e[3]], e[4], f[1], f[2], f[3], 0));
                    EXACT_DATE_TIME[d][e] = c
                }
                return b && (c = i(c, f[4], b)), c
            }, k = function (a, b) {
                for (var c = [], d = 0; b && d < b.length; d++)b[d][0] <= a && (b[d][1] >= a || b[d][0] === a && "only" === b[d][1] || "max" === b[d][1]) && c.push([a, b[d]]);
                return c
            }, l = function (a, b, d) {
                var e, f;
                return a.constructor !== Date ? (e = a[0], f = a[1], a = !d && EXACT_DATE_TIME[e] && EXACT_DATE_TIME[e][f] ? EXACT_DATE_TIME[e][f] : j(a, d)) : d && (a = i(a, c ? "u" : "w", d)), b.constructor !== Date ? (e = b[0], f = b[1], b = !d && EXACT_DATE_TIME[e] && EXACT_DATE_TIME[e][f] ? EXACT_DATE_TIME[e][f] : j(b, d)) : d && (b = i(b, c ? "u" : "w", d)), a = Number(a), b = Number(b), a - b
            }, m = d.getUTCFullYear();
        ...
        function getAbbreviation(a, b) {
            ...
            if (d.indexOf("%s") > -1) {
                var e;
                e = b ? "-" === b[7] ? "" : b[7] : "S", c = d.replace("%s", e)
            } else c = d.indexOf("/") > -1 ? d.split("/", 2)[b[6] ? 1 : 0] : d;
            return c
        }
        ...
        this.zoneFileBasePath = null, this.zoneFiles = ["africa", "antarctica", "asia", "australasia", "backward", "etcetera", "europe", "northamerica", "pacificnew", "southamerica"], this.loadingSchemes = {PRELOAD_ALL: "preloadAll", LAZY_LOAD: "lazyLoad", MANUAL_LOAD: "manualLoad"}, this.loadingScheme = this.loadingSchemes.LAZY_LOAD, this.loadedZones = {}, this.zones = {}, this.rules = {}, this.init = function (a) {
            var b, c = {async: !0}, d = this.loadingScheme === this.loadingSchemes.PRELOAD_ALL ? this.zoneFiles : this.defaultZoneFile || "northamerica", e = 0;
            for (var f in a)c[f] = a[f];
            if ("string" == typeof d)return this.loadZoneFile(d, c);
            b = c.callback, c.callback = function () {
                e++, e === d.length && "function" == typeof b && b()
            };
            for (var g = 0; g < d.length; g++)this.loadZoneFile(d[g], c)
            ...
            if (this.loadingScheme === this.loadingSchemes.LAZY_LOAD) {
                ...
                this.loadedZones[d] || this.loadZoneFile(d)
            }
            var e = getZone(a, b), f = e[0], g = getRule(a, e, c);
            g && (f = getAdjustedOffset(f, g));
